import { AgentHistory, AgentHistoryList } from "../views";
...
import z from "zod";
import { type ActionRegistry } from "../../controller/registry/views";
describe("Agent Tests", () => {
  ...
  it("should get errors", () => {
    ...
    expect(errors.find((i) => i?.includes("Failed"))).toBe(
      "Failed to extract completely"
    );
  });
  ...
  it("should get filtered model outputs", () => {
    ...
    if (filtered && filtered[0] && filtered[0].click_element) {
      expect(filtered[0].click_element.index).toBe(1);
    } else {
      // Fail the test if the structure is not as expected
      expect(filtered[0]?.click_element?.index).toBeDefined();
    }
  });
