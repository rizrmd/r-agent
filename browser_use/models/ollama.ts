...
function formatToolCall(additional: Message['additional_kwargs']): any[] | undefined {
    ...
    formatMessages(rawMessages: BaseMessage[], tool: StructedTool): RequestParams {
        ...
        for (const m of rawMessages) {
            ...
            if (typeof m.content === 'string') {
                ...
                newMsg.content = m.content.filter(i => i.type === 'text')
                    .map(i => i.text)
                    .join('\n');
                const images = m.content.filter(i => i.type === 'image_url')
