...
export class ChatGeminiAI extends BaseChatModel {
  ...
  formatMessages(
    ...
    for (const m of rawMessages) {
      ...
      if (m.type === "human") {
        geminiContents.push({
          role: "user",
          parts: [{ text: currentMessageTextContent }],
        });
      } else if (m.type === "ai") {
    ...
    const outputParams: RequestParams & {
      _gemini_system_instruction?: any;
      _gemini_tools?: any;
    } = {
    ...
    if (systemInstructions.length > 0) {
      outputParams._gemini_system_instruction = {
    ...
    if (tool) {
      ...
      if (
        ...
        const geminiFunctionDeclarations = langchainFormattedTools.tools.map(
          (t: any) => {
  ...
  async request(params: RequestParams): Promise<OpenAIMessage> {
    // Accepts RequestParams to be compatible with BaseChatModel
    const geminiSystemInstruction = (params as any)._gemini_system_instruction;
    const geminiTools = (params as any)._gemini_tools;

    const bodyForGemini = {
      contents: params.messages, // These are the geminiContents from formatMessages
      ...(geminiTools && { tools: geminiTools }), // These are Gemini-formatted tools from formatMessages
    ...
    if (
      ...
      if (part.text) {
        return { role: "assistant" as const, content: part.text };
            ...
            tool_calls: [
              {
                id: `call_${Date.now()}_${Math.random()
                  .toString(36)
                  .substr(2, 9)}`, // Generate unique ID for Gemini tool calls
    ...
    console.error(
      "Gemini API Error: Unexpected response structure",
      responseData
    );
