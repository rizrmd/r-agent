...
export class Controller<Context> {
  ...
  constructor(
    ...
    this.registry.action({
      ...
      func: async (params: z.infer<typeof SearchGoogleAction>, ctx: ActionRunContext) => {
    ...
    this.registry.action({
      ...
      func: (async (params: z.infer<typeof GoToUrlAction>, ctx: ActionRunContext) => {
        const page = await ctx.browser.get_current_page();
    ...
    this.registry.action({
      name: 'go_back',
      description: "Go back",
      func: async (params: z.infer<typeof NoParamsAction>, ctx: ActionRunContext) => {
    ...
    this.registry.action({
      ...
      func: async (params: z.infer<typeof ClickElementAction>, ctx: ActionRunContext) => {
        ...
        try {
          ...
          if (downloadPath) {
            ...
            msg = `🖱️  Clicked button with index ${params.index
    ...
    this.registry.action({
      ...
      func: async (params: z.infer<typeof SwitchTabAction>, ctx: ActionRunContext) => {
    ...
    this.registry.action({
      ...
      func: async (params: z.infer<typeof OpenTabAction>, ctx: ActionRunContext) => {
    ...
    this.registry.action({
      ...
      func: async (
        ...
        const prompt = `Your task is to extract the content of the page. You will be given a page and a goal`
          ...
          + `summarize the page. Respond in json format. Extraction goal: ${(goal as {goal: string}).goal || goal}, Page: ${pageContent}`;
    ...
    this.registry.action({
      ...
      func: async (params: z.infer<typeof SendKeysAction>, ctx: ActionRunContext) => {
        const page = await ctx.browser.get_current_page();
    ...
    this.registry.action({
      ...
      func: async (params: { index: number }, ctx: ActionRunContext): Promise<ActionResult> => {
        const page = await ctx.browser.get_current_page();
        const selectorMap = await ctx.browser.get_selector_map();
        ...
        try {
          ...
          for (const frame of page.frames()) {
            try {
              ...
              `);
              const options = await handler.evaluate<any>((handler: any, args) => handler(args), [handler, domElement.xpath]);
              if (options) {
              ...
              logger.debug(`Frame ${frameIndex} evaluation failed: ${String(frameE)}`);
            }
    ...
    this.registry.action({
      ...
      func: async (
        ...
        if (domElement.tag_name !== "select") {
          logger.error(
            `Element is not a select! Tag: ${domElement.tag_name}, Attributes: ${JSON.stringify(
        ...
        try {
          ...
          for (const frame of page.frames()) {
            try {
              ...
              const dropdownInfo = await handler.evaluate<any>((handler, xpath) => handler(xpath), [handler, domElement.xpath]);
              ...
              logger.error(`Frame ${frameIndex} attempt failed: ${String(frameE)}`);
