...
const CheckEmailSchema = z.object({
  ...
  folder: z.string().optional().default("inbox").describe("The email folder to check (e.g., 'inbox', 'spam', 'work')."),
  senderFilter: z.string().optional().describe("Filter emails by a specific sender."),
  unreadOnly: z.boolean().optional().default(true).describe("Whether to check only unread emails."),
  description: z.string().optional().describe("Description of this check email action."),
});
...
const SendEmailSchema = z.object({
  ...
  description: z.string().optional().describe("Description of this send email action."),
});
...
const BookCalendarSchema = z.object({
  ...
  startTime: z.string().datetime().describe("The start date and time for the event (ISO 8601 format)."),
  endTime: z.string().datetime().describe("The end date and time for the event (ISO 8601 format)."),
  participants: z.array(z.string().email()).optional().describe("List of participant email addresses."),
  description: z.string().optional().describe("Description of this book calendar action."),
});
...
// Schema for a list of assistant actions
const MultiActionAssistantSchema = z.object({
  actions: z.array(SingleAssistantActionSchema).min(1).describe("A list of assistant tasks to perform in sequence."),
});
...
const assistantTool = new StructuredTool({
  ...
  schema: MultiActionAssistantSchema,
});

// Stub functions for assistant actions
function checkEmailStub(params: z.infer<typeof CheckEmailSchema>): string {
  const logMessage = `Email Stub: Checking ${params.unreadOnly ? 'unread' : 'all'} emails in folder '${params.folder}'` +
...
function sendEmailStub(params: z.infer<typeof SendEmailSchema>): string {
  const logMessage = `Email Stub: Sending email to '${params.to}' with subject '${params.subject}'. Body: "${params.body.substring(0, 50)}..."`;
  console.log(logMessage);
  return `Email successfully sent to ${params.to}.`;
}

function bookCalendarStub(params: z.infer<typeof BookCalendarSchema>): string {
  const logMessage = `Calendar Stub: Booking event '${params.title}' from ${params.startTime} to ${params.endTime}.` +
    (params.participants && params.participants.length > 0 ? ` Participants: ${params.participants.join(', ')}.` : '');
  console.log(logMessage);
...
async function runPersonalAssistantSimulation() {
  const messages = [
    ...
    new HumanMessage({
      content: "Please check my main inbox for new emails. If there's an email from 'jane.doe@example.com' about 'Project Alpha', reply to her saying 'Got it, will review soon.' and then book a 30-minute meeting with her for tomorrow at 10 AM titled 'Project Alpha Sync'.",
    }),
  ];

  try {
    console.log("User request:", (messages[messages.length -1] as HumanMessage).content);
    ...
    if (!response.success) {
      ...
      throw new Error("Failed to perform assistant tasks due to invalid LLM output. Check logs for details.");
    }

    console.log("\nLLM proposed actions:", JSON.stringify(response.data.actions, null, 2));
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      console.log(`\nStep ${i + 1}: ${action.description || `Executing ${action.actionType}`}`);
      
      switch (action.actionType) {
        case "checkEmail":
          lastActionResult = checkEmailStub(action as z.infer<typeof CheckEmailSchema>);
          break;
        case "sendEmail":
          ...
          lastActionResult = sendEmailStub(action as z.infer<typeof SendEmailSchema>);
          break;
        case "bookCalendar":
          lastActionResult = bookCalendarStub(action as z.infer<typeof BookCalendarSchema>);
          break;
        default:
          console.error(`Error: Unknown action type at runtime: ${(action as any).actionType}`);
